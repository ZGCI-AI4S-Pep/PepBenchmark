...
class SingleTaskDataset(BaseDataset):
    ...
    def get_data(self, format = "df", split = None, fold_seed = 1, split_ready = True):
        ...
        if split_ready == False:
            ...
            df = PepConvert(df, self.raw_format, self.convert_format, self.feature_type, self.all_nature)
            return df

        else:
            assert split in ['Random_split', 'Homology_based_split'],\
                "split should be 'Random_split' or 'Homology_based_split'."

            assert fold_seed in [1, 2, 3, 4, 5],\
                "fold_seed should be 1, 2, 3, 4, or 5."

            print('Loading split ready dataset')
            dict_split = {}
            for split_type in ['train', 'valid', 'test']:
                df_split = pd.read_csv(
                get_dataset_path(self.dataset_name, split=split, fold_seed=1, type=split_type)
            )
                dict_split[split_type] = PepConvert(df_split, self.raw_format, self.convert_format, self.feature_type, self.all_nature)
