...
from..utils import random_split, homology_based_split, cold_split
...
class BaseDataset:
    ...
    def get_split(self,
        ...
        if method == 'random':
            ...
            split = homology_based_split(df, frac=frac, seq_alighment=seq_alighment, sim_threshold=sim_threshold, seed=seed)
    ...
    def deduplicate(self,  mode: str = 'average',
                    dedup_keys: Optional[List[str]] = None) -> pd.DataFrame:

        assert mode in ['min', 'max', 'average', 'leave_one', 'majority', 'remove_all'], f"Invalid mode: {mode}"

        dedup_keys = dedup_keys
        df = self.dataset.copy()

        if self.type == 'regression':
            assert mode in ['min', 'max', 'average'], f"Invalid mode: {mode} for regression"
            ...
            assert mode in ['majority', 'leave_one','remove_all'], f"Invalid mode: {mode} for binary classification"
            ...
            df = df.groupby(dedup_keys, as_index=False).apply(resolve_conflict).reset_index(drop=True)
    ...
    def augmentation(self, method: str = 'random_replace', ratio: float = 0.02):
        from ..utils import random_replace, random_delete, random_replace_with_A, random_swap, random_insertion_with_A, \
            combine
        assert method in ['random_replace', 'random_delete', 'random_replace_with_A', 'random_swap', 'random_insertion_with_A'], f"Invalid method: {method}"
        if method == 'random_replace':
            ...
            new_inputs, new_labels = random_replace_with_A(self.sequence, self.label, ratio)
        elif method == 'random_swap':
            new_inputs, new_labels = random_swap(self.sequence, self.label, ratio)
        elif method == 'random_insertion_with_A':
            new_inputs, new_labels = random_insertion_with_A(self.sequence, self.label, ratio)

        self.sequence, self.label = combine(self.sequence, self.label, new_inputs, new_labels)
        self.dataset = pd.concat([pd.DataFrame(self.sequence, columns=['sequence']), self.label], axis=1)
        return self


    def binarize(self,threshold: float = 100, order: str = 'descending'):
        if self.type == 'binary_classification':
            raise NotImplementedError("Binarization not implemented for binary classification.")
        ...
        if len(np.unique(self.label)) == 2:
            ...
            print( "Binariztion using threshold " + str(threshold) +
                ", default, we assume the smaller values are 1 "
                "and larger ones is 0, you can change the order "
                "by 'binarize(order = 'ascending')'")
            if order == 'ascending':
                self.label = np.array([1 if i else 0 for i in np.array(self.label) > threshold])
            ...
            if (np.unique(self.label).reshape(-1,).shape[0] < 2):
                raise AttributeError(
    ...
    def pep_distribution(self):
        ...
        from ..visualization import plot_peptide_distribution, plot_peptide_distribution_spited
    ...
    def print_stats(self):
        """print statistics"""
        print(
            "There are " + str(len(self.sequence)) + " sampels, and " + str(len(np.unique(self.sequence))) + " unique peptide sequences in the dataset."
        )
