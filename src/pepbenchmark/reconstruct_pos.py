...
dataset_names = [
    name for name in dataset_names if name not in ["solubility", "nonfouling", "ttca"]
]
...
for dataset in dataset_names:
    ...
    pos_pool_path = "/home/dataset-assist-0/rouyi/rouyi/Projects/PepBenchmark/data_share/peptide_dataset/official_dataset_50_v1/negative_pool/peptidepedia.csv"
    pos_pool = pd.read_csv(pos_pool_path)
    pos_pool.columns = [col.lower() for col in pos_pool.columns]
    df = pd.read_csv(file_path + "/positive_sequences.csv")
    ...
    df_deduplicated.to_csv(file_path + "/pos_seqs_50.csv", index=False)
    print(
