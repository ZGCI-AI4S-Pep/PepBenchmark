...
class DatasetBuilder:
    ...
    def _get_tokenizer_max_length(self) -> Optional[int]:
        max_len = getattr(self.tokenizer, "model_max_length", None)
        # Some tokenizers use a very large sentinel value for "unlimited".
        if max_len is None:
            return None
        try:
            max_len = int(max_len)
    ...
    def _resolve_effective_max_length(self, candidate_max_length: int) -> int:
        tokenizer_max = self._get_tokenizer_max_length()
        if tokenizer_max is None:
            return candidate_max_length
        if candidate_max_length > tokenizer_max:
            logger.warning(
                f"Requested max_length={candidate_max_length} exceeds tokenizer/model limit "
                f"{tokenizer_max}; clamping to {tokenizer_max}."
            )
    ...
    def infer_max_length(
        ...
        if self.max_length is not None:
            return self._resolve_effective_max_length(self.max_length)
        all_smiles: List[str] = []
        ...
        inferred_max_length = max(token_lengths) + extra_padding
        return self._resolve_effective_max_length(inferred_max_length)
    ...
    def build_dataset(self, features: Dict[str, Any], smiles_key: str, label_key: str, max_length: int) -> TokenizedSMILESDataset:
        ...
        effective_max_length = self._resolve_effective_max_length(max_length)
        encodings = self.tokenizer(smiles, add_special_tokens=True, truncation=True, max_length=effective_max_length)
...
def build_smiles_model(
    ...
    **kwargs,
    ...
    model_name = kwargs.pop("model_name", kwargs.pop("model", "seyonec/ChemBERTa-zinc-base-v1"))
