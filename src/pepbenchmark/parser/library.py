...
from pepbenchmark.parser.entity import Residue
from pepbenchmark.parser.utils import aspath
...
class ConcatLibrary(Library):
    def __init__(self, id: str, libs: Iterable[Library]) -> None:
        ...
        for d in self.child_dict.values():
            assert isinstance(
                d, Library
            ), "ConcatLibrary only accepts Library objects"
        self.cumulative_sizes = self.cumsum(self.child_dict.values())
