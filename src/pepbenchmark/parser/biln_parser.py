...
# BILN format parsing and serialization
class BilnSyntaxError(ValueError):
    """Error during BILN string parsing (syntax). Thrown during BILN string parsing for syntax errors, facilitating exception location and handling."""
...
class ParsedBilnMonomerInfo(ParsedMonomerInfo):
    ...
    # Used to record detailed connection and separator information for each monomer during BILN parsing.
...
class BilnParser(Parser):
    ...
    Responsible for parsing a BILN string into an intermediate data structure."""
...
class BilnSerializer(Serializer):
    ...
    def serialize(self, parsed_data: ParsedData) -> str:
        ...
        for idx, (m_idx, exp_conns, sep) in enumerate(biln_parts):
            ...
            abbr = m.res.m_abbr
            # When monomer abbreviation contains characters that would
            # conflict with BILN syntax (e.g., '-', '(' or ')'), wrap it.
            if re.search(r"[-()]", abbr):
                abbr = f"[{abbr}]"
            conns_str = "".join(
...
if __name__ == "__main__":
    ...
    # Create a BILN parser with the given monomer library `lib`, then use it to parse the BILN string `biln`, and finally obtain the structured parsed result `parsed_data`.
