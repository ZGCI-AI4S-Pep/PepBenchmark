...
def main():
    ...
    parser.add_argument('--dataset_list', type=list, default=["ACE_APML","ACP_APML","Aox_APML","DLAD_BioDADPep","DPPIV_APML","Neuro_APML","QS_APML","TTCA_APML"],
                        help='dataset name (default: ogbg-molhiv)')
    parser.add_argument('--activity', type=str, default='Theraputic-Other',
                        help='activity type (default: ADME)')
    parser.add_argument('--model_list', type=list, default=['gcn','gat','gin','transformer'],
    ...
    parser.add_argument('--splits', type=list, default=['random0','random1','random2','random3','random4','mmseqs20','mmseqs21','mmseqs22','mmseqs23','mmseqs24'],
    ...
    os.makedirs('./gnn_epoch_results', exist_ok=True)
    os.makedirs('./gnn_results', exist_ok=True)

    for dataset in args.dataset_list:
        root_path = "/home/dataset-assist-0/rouyi/rouyi/Projects/PepBenchmark/data_share/peptide_dataset/original_dataset/"+args.activity+"/"+dataset+"/"  # Replace with your actual file path
        smiles, labels = datasets(root_path, args.is_smiles)  # Replace with your actual file path
        
        for i in args.splits:
            ### automatic dataloading and splitting
            train_dataset = PeptideDataset(smiles, labels, root_path+i[:-1]+'_split.json', 'seed_'+i[-1], 'train','./graph_data/'+args.activity+"/"+dataset+"/")
            valid_dataset = PeptideDataset(smiles, labels, root_path+i[:-1]+'_split.json', 'seed_'+i[-1], 'valid','./graph_data/'+args.activity+"/"+dataset+"/")
            test_dataset = PeptideDataset(smiles, labels, root_path+i[:-1]+'_split.json', 'seed_'+i[-1], 'test','./graph_data/'+args.activity+"/"+dataset+"/")
            ...
            for j in args.model_list:
                ...
                for k in [0.001]:
                    for l in [0.0001]:
                        ...
                        for epoch in range(1, args.epochs + 1):
                            ...
                            with open('./gnn_epoch_results/'+dataset+'_'+i+'_'+args.gnn+'_lr'+str(k)+'_wd'+str(l)+'.txt', 'a') as f:  
                ...
                # Only write results if we have valid results
                if temp_best_train is not None and temp_best_val is not None and temp_best_test is not None:
                    with open('./gnn_results/'+dataset+'.txt', 'a') as fi:
