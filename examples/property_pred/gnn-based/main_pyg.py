...
from dataset import PeptideDataset, datasets
from gnn import GNN
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            if "classification" in task_type:
                loss = cls_criterion(
                    pred.to(torch.float32)[is_labeled].squeeze(),
                    batch.y.to(torch.float32)[is_labeled].squeeze(),
                )
            else:
                loss = reg_criterion(
...
def main():
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="DLAD_BioDADPep",
        help="dataset name (default: ogbg-molhiv)",
    )
    parser.add_argument(
        "--activity", type=str, default="ADME", help="activity type (default: ADME)"
    )
    ...
    for i in (
        ...
        for j in ["gcn", "gin", "gat", "transformer"]:
                ...
                for l in [0.01, 0.001, 0.0001, 1e-5, 1e-6, 0]:
                    ...
                    for epoch in range(1, args.epochs + 1):
                        ...
                        with open(
                            ...
                            f.write(
                                "Epoch: {}, Train Acc: {:.4f}, Train Pre: {:.4f}, Train Rec: {:.4f}, Train F1: {:.4f}, Train ROC AUC: {:.4f}".format(
                                    epoch,
                                    train_perf["acc"],
                            ...
                            f.write(
                                "Epoch: {}, Valid Acc: {:.4f}, Valid Pre: {:.4f}, Valid Rec: {:.4f}, Valid F1: {:.4f}, Valid ROC AUC: {:.4f}".format(
                                    epoch,
                                    valid_perf["acc"],
                                    valid_perf["precision"],
                                    valid_perf["recall"],
                                    valid_perf["f1"],
                                    valid_perf["roc_auc"],
                                )
                            ...
                            f.write(
                                "Epoch: {}, Test Acc: {:.4f}, Test Pre: {:.4f}, Test Rec: {:.4f}, Test F1: {:.4f}, Test ROC AUC: {:.4f}".format(
                                    epoch,
                                    test_perf["acc"],
                                    test_perf["precision"],
                                    test_perf["recall"],
                    ...
                    print(
                        "Best validation score: {}".format(
                            valid_curve["roc_auc"][best_val_epoch]
                        )
                    )
                    print(
                        "Test score: {}".format(test_curve["roc_auc"][best_val_epoch])
                    )

                    temp_best_train = {
                        "acc": train_curve["acc"][best_train_epoch],
                        "precision": train_curve["precision"][best_train_epoch],
                        "recall": train_curve["recall"][best_train_epoch],
                        "f1": train_curve["f1"][best_train_epoch],
                        "roc_auc": train_curve["roc_auc"][best_train_epoch],
                    }
                    temp_best_val = {
                        "acc": valid_curve["acc"][best_val_epoch],
                        "precision": valid_curve["precision"][best_val_epoch],
                        "recall": valid_curve["recall"][best_val_epoch],
                        "f1": valid_curve["f1"][best_val_epoch],
                        "roc_auc": valid_curve["roc_auc"][best_val_epoch],
                    }
                    temp_best_test = {
                        "acc": test_curve["acc"][best_val_epoch],
                        "precision": test_curve["precision"][best_val_epoch],
                        "recall": test_curve["recall"][best_val_epoch],
                        "f1": test_curve["f1"][best_val_epoch],
                        "roc_auc": test_curve["roc_auc"][best_val_epoch],
                    }

            with open("./results/" + args.dataset + ".txt", "a") as fi:
                fi.write(
                    "Model: {}, Split: {}, Train Acc: {:.4f}, Train Pre: {:.4f}, Train Rec: {:.4f}, Train F1: {:.4f}, Train ROC AUC: {:.4f}".format(
                        args.gnn,
                        i,
                        temp_best_train["acc"],
                        temp_best_train["precision"],
                        temp_best_train["recall"],
                        temp_best_train["f1"],
                        temp_best_train["roc_auc"],
                    )
                    + "\n"
                )
                fi.write(
                    "Model: {}, Split: {}, Valid Acc: {:.4f}, Valid Pre: {:.4f}, Valid Rec: {:.4f}, Valid F1: {:.4f}, Valid ROC AUC: {:.4f}".format(
                        args.gnn,
                        i,
                        temp_best_val["acc"],
                        temp_best_val["precision"],
                        temp_best_val["recall"],
                        temp_best_val["f1"],
                        temp_best_val["roc_auc"],
                    )
                    + "\n"
                )
                fi.write(
                    "Model: {}, Split: {}, Test Acc: {:.4f}, Test Pre: {:.4f}, Test Rec: {:.4f}, Test F1: {:.4f}, Test ROC AUC: {:.4f}".format(
                        args.gnn,
                        i,
                        temp_best_test["acc"],
                        temp_best_test["precision"],
                        temp_best_test["recall"],
                        temp_best_test["f1"],
                        temp_best_test["roc_auc"],
                    )
                    + "\n"
                )
